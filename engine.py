...
def train_one_epoch(model: torch.nn.Module, original_model: torch.nn.Module, feature_prefix_gt, feature_prefix_et,
    ...
    for input, target in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        if task_id != 0 and not args.no_pgp:
            for k, (m, params) in enumerate(model.module.named_parameters()):
                if m == "g_prompt":
                    # print('*'*100)
                    # print('g_prompt####:  params.grad.data.shape', params.grad.data.shape)
                    ...
                    # print('lz0 ',lz0,' sz0 ',sz0,' lz1 ',lz1, ' sz1 ',sz1)
                if m == "e_prompt.prompt":
                    # print('*'*100)
                    # print('e_prompt####:  params.grad.data.shape', params.grad.data.shape)
                    ...
                    # print('params.grad.data[0][0][prompt_id]', params.grad.data[0][0][prompt_id].shape)
                    # print('params.grad.data[1][0][prompt_id]', params.grad.data[1][0][prompt_id].shape)
                    # print('params.grad.data[2][0][prompt_id]', params.grad.data[2][0][prompt_id].shape)
    ...
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}, prompt_id
...
def train_and_evaluate(model: torch.nn.Module, model_without_ddp: torch.nn.Module, original_model: torch.nn.Module, 
    ...
    for task_id in range(args.num_tasks):
        print('='*20, 'task_id', task_id, 'args.num_tasks', args.num_tasks)
        ...
        for epoch in range(args.epochs):            
            train_stats, prompt_id = train_one_epoch(model=model, original_model=original_model, feature_prefix_gt=feature_prefix_gt,
        ...
        # ###############################
        # prefix_grad_g, prefix_grad_e = [], [None,None,]
        # # if epoch == args.epochs - 1:
        # for k, (m, params) in enumerate(model.module.named_parameters()):
        #     if m == "g_prompt":
        #         # if params.grad.data.size(0) == 24:
        #         print('g_prompt:  params.grad.data.shape', params.grad.data.shape)
        #         prefix_grad_g.append(params.grad.data.detach().cpu().numpy())

        #     if m == "e_prompt.prompt":
        #         # if params.grad.data.size(0) == 24:
        #         print('e_prompt.prompt:  params.grad.data.shape', params.grad.data.shape)
        #         prefix_grad_e.append(params.grad.data.detach().cpu().numpy())
        # ##################################

        if not args.no_pgp:
            prefix_rep_g, prefix_rep_e, prefix_grad_g, prefix_grad_e = memory.get_prefix_matrix(data_loader[task_id]['mem'], model, device, prompt_id.item())

            threshold = 0.50
            feature_prefix_g, grad_stack_list_g, space_list_g, feature_list_g = memory.update_memory_prefix(prefix_rep_g, threshold, feature_prefix_g, prefix_grad_g, grad_stack_list_g, space_list_g, feature_list_g)
            feature_prefix_e, grad_stack_list_e, space_list_e, feature_list_e = memory.update_memory_prefix(prefix_rep_e, threshold, feature_prefix_e, prefix_grad_e, grad_stack_list_e, space_list_e, feature_list_e)
            ...
            for layer in feature_prefix_g:
                for item in feature_prefix_g[layer]:
                    ...
                    Uf = torch.mm(temp_feature, temp_feature.transpose(0,1))
